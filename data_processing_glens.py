...
def get_land_mask(filepath, var):
    ...
    mask = mask[5,5,:,:]
    mask = mask.where(mask >= -1000, np.nan, drop=False)*0. + 1
...
def get_data(DATA_DIRECTORY):
    ...
    for ens in range(0,21):
        ...
        filename_arise = DATA_DIRECTORY + 'feedback.' + member_text + '.T.r90x45.shift.annual.nc'
        daa = xr.open_dataset(filename_arise)
        daa = process_variable("T", daa, (2020,2097))
