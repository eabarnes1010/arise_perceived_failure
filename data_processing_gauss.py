...
#         filename_ssp = DATA_DIRECTORY + 'control.' + member_text + '.T.r90x45.shift.annual.nc'
#         das = xr.open_dataset(filename_ssp)
        
#         das = process_variable("T", das, (2010,2097))
...
def get_data(DATA_DIRECTORY, var):
    da_all = None

    for ens in np.arange(0,3):
        ...
        # filename_ssp = DATA_DIRECTORY + 'control.' + member_text + '.T.r90x45.shift.annual.nc'
        # das = xr.open_dataset(filename_ssp)
        # das = process_variable("T", das, (2010,2019))

        filename_arise = DATA_DIRECTORY + 'b.e21.BWSSP245.f09_g17.release-cesm2.1.3.WACCM-MA-1deg.SSP245-MA-GAUSS-LOWER-1.0.' + member_text + '.cam.h0.' + var + '.203501-207012_annual.nc'
        try:
            filename_arise = DATA_DIRECTORY + 'b.e21.BWSSP245.f09_g17.release-cesm2.1.3.WACCM-MA-1deg.SSP245-MA-GAUSS-LOWER-1.0.' + member_text + '.cam.h0.' + var + '.203501-207012_annual.nc'           
            da = xr.open_dataset(filename_arise)
        except:
            filename_arise = DATA_DIRECTORY + 'b.e21.BWSSP245.f09_g17.release-cesm2.1.3.WACCM-MA-1deg.SSP245-MA-GAUSS-LOWER-1.0.' + member_text + '.cam.h0.' + var + '.203501-206912_annual.nc'           
            da = xr.open_dataset(filename_arise)
            
        da = process_variable(var, da, (2035,2069))
